...
if __name__ == "__main__":
    ...
    parser.add_argument("--imgs", default="./imgs", help="path to images directory")
    ...
    imgs=[]
    images_list = sorted(glob.glob(f'{opt.imgs}/*'))
    ...
    cur_ava = 0    
    change_avatar(fa, imgs[cur_ava])
    passthrough = False

    cv2.namedWindow('cam', cv2.WINDOW_GUI_NORMAL)
    cv2.namedWindow('DeepFakeCamera', cv2.WINDOW_GUI_NORMAL)
    cv2.moveWindow('cam', 0, 0)
    cv2.moveWindow('DeepFakeCamera', 600, 0)
    ...
    while True:
        ...
        if find_keyframe:
            if is_new_frame_better(fa, imgs[cur_ava], frame, device):
        ...
        if passthrough:
            ...
            pred = predict(frame, imgs[cur_ava], opt.relative, opt.adapt_scale, fa, device=device)
            if not opt.no_pad:
        ...
        preview_frame = cv2.addWeighted( imgs[cur_ava][:,:,::-1], overlay_alpha, frame, 1.0 - overlay_alpha, 0.0)
